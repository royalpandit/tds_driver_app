...
class OtpScreen extends StatefulWidget {
  ...
  const OtpScreen({super.key, required this.contact, required this.userType});
...
class _OtpScreenState extends State<OtpScreen> {
  ...
  Widget _buildKey(String value) {
    return _KeyButton(label: value, onTap: () => _onKeyPressed(value));
  }

  Widget _iconKey(IconData icon, VoidCallback onTap, {bool filled = false}) {
    return _KeyButton(icon: icon, filled: filled, onTap: onTap);
  }
}
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Container(
        ...
        child: icon != null
            ? Icon(icon, color: filled ? Colors.white : Colors.black, size: 24)
            : Text(
                label!,
                style: GoogleFonts.poppins(
                  fontSize: 20,
